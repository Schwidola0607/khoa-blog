...
This blog series assumes that model weights can fit on a small number of GPUs within a single node (2 or 4). If you need all 8 GPUs in a node or multi-node deployment just to fit the weights, Sequence Parallelism might not be the sole solution - you may need to use it in conjunction with other parallelism techniques. 
...
1. All-Reduce communication volume increases with more devices
...
1. Attention LayerNorm acts on the sharded input sequence

1. All-Gather the sharded input sequence into the full sequence

1. Perform the Attention Block (QKV proj, Attention, and O proj) in traditional TP fashion

1. Reduce partial results of O proj and scatter them along the input sequence

1. MLP LayerNorm acts on the sharded input sequence

1. All-Gather the sharded input sequence into the full sequence

1. Perform the MLP Block (expand, activation, and shrink) in traditional TP fashion

1. Reduce partial results of shrink and scatter them along the input sequence for the next layer's Attention LayerNorm
...
1. Pass the sharded input sequence to QKV proj to get sequence-sharded Q, K, V

1. Perform an All-To-All to reshard the sequence-sharded Q, K, V into head-sharded Q, K, V

1. Each device performs attention on a subset of heads

1. Perform an All-To-All to reshard the head-sharded `attn_output` back to sequence-sharded

1. Pass the sequence-sharded output to O proj, LayerNorm, and MLP block
